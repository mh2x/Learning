        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('description')
                    ...
                    ->formatStateUsing(fn ($state): string => Str::before($state, ', ')),
                Tables\Columns\TextColumn::make('id')
                    ->label(__('Permissions'))
                    ->formatStateUsing(function ($state) {
                        $permissions = Role::find($state)->permissions;
                        $count = Permission::count();
                        if($count==$permissions->count()){
                            return __('All Permissions');
                        }else if($permissions->count()==0){
                            return __('No Permissions');
                        }else{
                            return $permissions?->take(3)->pluck('name')->map(function ($name) {
                                if (Str::contains($name,"_shield::")) {
                                    return Str::replaceFirst('_shield::', ' ', $name);
                                }else{
                                    return Str::replaceFirst('_', ' ', $name);
                                }
                            })->implode(', ');
                        };
                    }),
                Tables\Columns\TextColumn::make('users_count')
                    ...
                    ->counts('users')
                    ->formatStateUsing(fn ($state): string =>  $state==1?$state." " . __('User') :$state." " . __('Users'))
                    ->colors(['success']),
